...
export default function Analytics() {
  ...
  // Admin access restricted empty state
  if (isAdminRestricted) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-6 select-none max-w-lg mx-auto">
        <div className="h-14 w-14 rounded-2xl bg-primary-950/60 border border-primary-500/25 flex items-center justify-center text-primary-400 mb-5 shadow-xl shadow-primary-500/5">
          <Lock size={22} className="animate-pulse" />
        </div>
        <h2 className="text-lg font-bold text-text-primary tracking-tight">Admin Credentials Required</h2>
        <p className="text-xs text-text-secondary mt-2.5 leading-relaxed font-medium">
          Analytics dashboard displays global platform logs, rate limiter activity, and request telemetry. To view these metrics, please contact your systems administrator or authenticate as an <span className="text-primary-400 font-semibold">ADMIN</span> user.
        </p>
        <div className="flex gap-4 mt-8 w-full">
          <Button onClick={() => window.location.href = '/dashboard'} className="flex-1 bg-gradient-to-br from-background-card to-[#1c1c1e] hover:from-[#1c1c1e] hover:to-[#242426] border border-border text-text-primary">
            Back to Dashboard
          </Button>
